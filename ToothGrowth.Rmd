...
This document describes a statisical summary performed on the ToothGrowth dataset. The study performed by E.W. Crampton analyzes the impact of Vitamin C on the growth of the incisors of guinea pigs. The source of the data found in the ToothGrowth dataset is C.I Bliss (1952) *The Statistics of Bioassay.* Academic Press. A copy of the original study is avaiable in *The Journal OF Nutrition* at http://jn.nutrition.org/content/33/5/491.full.pdf. 
...
(2) The dataset supports a hpothesis that the delivery of Vitamin C delivered by orange juice promotes greater tooth growth than aqueous solution, measured by a greater mean tooth length.
...
The analysis begins by taking an exploratory look at the data. The table that follows shows the average tooth length (microns) by supplement and dose (milligrams).
...
A cursory look at the distribution of tooth growth by supplement shows that the median tooth growth is higher for the guinea pigs that received Vitamin C from orange juice, versus those who receive Vitamin C via an aqueous solution. The boxplot also shows that the dispersion of tooth growth is greater for the guinea pigs who received the aqueous solution versus orange juice. We will examine whether this effect is statistically significant.

```{r echo=FALSE, message=TRUE, warning=FALSE, fig.width=10}
g<-ggplot(ToothGrowth, aes(x= supp, y = len, fill=supp))
g<-g+geom_boxplot()
g<-g+ggtitle("Cursory View of the Impact of Orange Juice & Vitamin C
             on the Growth of Teeth for Guinea Pigs")
g

```


The results of the boxplot are supported by the following line charts that shows the relationship between length, dose, and supplement. It would appears from these charts that at lower dosages, Vitamin C delivered via orange juice promotes tooth growth more effectively than Vitamin C delivered via aqueous solution .
...
To test the first hypothesis, that the dataset supports a hypothesis that the administration of Vitamin C promotes tooth growth, I computed six test statistics measuring the difference in the mean tooth length by dose and supplement. These statistics compare the difference in mean tooth length for pairings of doses, by supplement. The test statistics labeled Tstat(x), where x=1,...,6, represent the following comparison:
...
colnames(stat)<-c("Lower Bound","Upper Bound","P-value")
stat
```

For each test statistic the Null hypothesis (i.e., that the mean difference in tooth length is zero) is rejected, since zero is not present in any of the confidence intervals and the p_values are smaller than the 5% test level. Based on these statistics, I conclude that Vitamin C administered by either method, orange juice or aqueous solution promotes tooth growth, given the data in the study. In addition, the amount of growth is positively correlated to the amount of Vitamin C delivered.

To address the second hypothesis, *that the dataset supports a hypothesis that the delivery method, orange juice or an aqueous solution of ascorbic acid, promotes different level of tooth growth, measured by a greater mean tooth length*, I computed three test statistics mesuring the mean difference in tooth length given the same dose level, but different delivery method. The test statistics labeled Tstat(x), where x=7,...9, represent the following comparison:
...
The second hypothesis postulated that Vitamin C delivered via orange juice promoted greater tooth growth than Vitamin C delivered via an aqueous solution. The test statistics for this hypothesis compared the mean growth of the two delivery methods at the same dose level. We found that at 0.5 mg, and 1.0 mg, we were able to reject the Null hypothesis that the mean difference in tooth growth was zero. Therefore, the alternative hypothesis would indicate that at 0.5 mg and 1.0 mg, Vitamin C delivered via orange juice promotes greater tooth growth than Vitamin C delivered via an aqueous solution. However, we could not reject the Null hypothesis at the 2.0 mg dose level. So it is not statistically possible to say that Vitamin C delivered via orange juice promotes greater tooth growth than Vitamin C delivered via an aqueous solution.

###Appendix

I used simulation to bootstrap distributions to test the two hypotheses. For the first hypothesis, I created distributions to examine Tsat1 (compares the mean length from aqueous solution at 2.0 mg - mean length from aqueous solution at 0.5 mg) and Tstat4 (compares the mean length from orange juice at 2.0 mg - mean length from orange juice at 0.5 mg).

I then used simulation to test the hpothesis that Vitamin C delievered via orange juice at 2.0 mg promoted greater tooth growth than Vitamin C delivered via aqueous solution at 2.0 mg.

In the simulations that follow the black vertical line is drawn at the observed test statistic. The fact that all of the outcomes in the histogram fall to the left of the line for the first two charts indicate that the p_value is below the selected 5% error rate, and supports rejection of the Null hypothesis (in both cases the P-Value was zero. In the third chart the vertical line is much closer to the center of the distribution and corresponds to a p-value of 0.5, which is above the 5% error rate. This supports the fact that the Null hypothesis cannot be rejected.
...
perm<-sapply(1:10000, function(i) testStat(OJy, sample(OJgroup)))
mean(perm>OJobsstat)
g<-ggplot(data.frame(perm), aes(x=perm, color="perm"))
g<-g+geom_histogram(bins = 100)
g<-g+geom_vline(xintercept = OJobsstat, lwd =2)
g<-g+ggtitle("Permutation of ToothGrowth Dataset: Orange Juice Dosage 2.0 vs. 0.5")
g
...
perm<-sapply(1:10000, function(i) testStat(VCy, sample(VCgroup)))
mean(perm>VCobsstat)
g<-ggplot(data.frame(perm), aes(x=perm, color="perm"))
g<-g+geom_histogram(bins = 100)
g<-g+geom_vline(xintercept = VCobsstat, lwd =2)
g<-g+ggtitle("Permutation of ToothGrowth Dataset: Viitamin C Dosage 2.0 vs. 0.5")
g
...
perm<-sapply(1:10000, function(i) testStat(y, sample(group)))
mean(perm>obsstat)
g<-ggplot(data.frame(perm), aes(x=perm, color="perm"))
g<-g+geom_histogram(bins = 100)
g<-g+geom_vline(xintercept = obsstat, lwd =2)
g<-g+ggtitle("Permutation of ToothGrowth Dataset: Orange Juice vs. Vitamin C at a Dosage of 2.0")
g
